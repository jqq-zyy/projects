				...
				<div class="right-content-wrap">
					...
					<div class="admin-data-items">
						<div class="right-header">
							<div class="base-info company-info">{{g.lang("基本信息")}}</div>
							<div class="base-info company-info" @click="onClick_goCompanyBtn"
								 v-show="!roleId">{{g.lang("企业认证信息")}}
							</div>
							<div class="base-info current-item  company-info"  @click="onClick_setAlpayBtn">退款账号</div>
						</div>
						<div class="right-body personal-box">
							<div class="detail-info">
								<div class="title">{{g.lang("用户名")}}</div>
								<div class="title-content name">{{name}}</div>
							</div>
							<div class="detail-info">
								<div class="title">{{g.lang("手机号")}}</div>
								<div class="title-content">
									<span class="phone-number name" v-text="companyTel" v-show="!isModTel"></span>
									<input type="text" v-model="companyTel" v-show="isModTel">
									<div class="mod-number pointer" :class="{'isDisable':isModTel}"
										 @click="onClick_modTelBtn">修改
									</div>
								</div>
							</div>
